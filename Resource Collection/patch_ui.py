...
with open(r"d:\DANIEL\web dev 1\Resource Collection\index.html", "r", encoding="utf-8") as f:
    html = f.read()

# 1. WRAP LAYOUT (SHELL + SIDEBAR + HEADER)
nav_regex = re.compile(r'<nav class="bg-red-600.*?</nav>', re.DOTALL)
main_container_regex = re.compile(r'<div class="max-w-6xl mx-auto p-4 md:p-6 mt-4">')
...
html = main_container_regex.sub(app_shell_start, html) 

html = html.replace('</body>', '\n            </div>\n        </main>\n    </div>\n</div>\n</body>')

# 2. HAPUS TAMBAH AKUN DI DASHBOARD & GABUNGKAN KE KELOLA AKSES
tambahakun_menu_regex = re.compile(r'<div id="menuTambahAkun".*?</div>', re.DOTALL)
html = tambahakun_menu_regex.sub('', html)

old_manage_access_regex = re.compile(r'<div id="manageAccessSection".*?<!-- SECTION 5: TAMBAH AKUN \(RBAC\) -->', re.DOTALL)
...
"""
html = old_manage_access_regex.sub(new_manage_access, html)

add_account_section_end_regex = re.compile(r'<!-- MODAL EDIT AKSES \(RBAC MATRIX\) -->', re.DOTALL)
html = add_account_section_end_regex.sub("""
...
# JS Modifikasi
js_marker = "let userToken = '';"
html = html.replace(js_marker, """let userToken = '';
...
document.addEventListener('click', function(event) {
    ...
    if (dropdown && !dropdown.classList.contains('hidden')) {
        // If click is not inside dropdown and not the button itself
...
login_succ = re.compile(r'currentUserData = resp\.data;')
html = login_succ.sub("""currentUserData = resp.data; depoInfoData = resp.data.depoInfo;
...
js_table_render_regex = re.compile(r'function renderManageAccessTable\(data\).*?\}', re.DOTALL)
...
function editRoleMatrix(role) {
    ...
    document.getElementById('editRoleDisplay').textContent = "WARNING SYSTEM: Perilaku modifikasi melimpahi kewenangan pengguna secara radikal.";
...
html = js_table_render_regex.sub(card_render_js, html)

html = html.replace('<div>\n                            <label class="block text-xs font-bold text-gray-700 mb-1">Aktivasi Akun (Kolom I)</label>', '<div id="editActivationBlock">\n                            <label class="block text-xs font-bold text-gray-700 mb-1">Aktivasi Akun (Kolom I)</label>')
html = html.replace('<div>\n                            <label class="block text-xs font-bold text-gray-700 mb-1">Menu Claim System (Kolom J)</label>', '</div><div id="manageRBACBlock" class="hidden w-full space-y-4">\n                        <div>\n                            <label class="block text-xs font-bold text-gray-700 mb-1">Menu Claim System (Kolom J)</label>')
html = html.replace('<!-- Hak Tambah Akun -->\n                    <div class="bg-white p-4 border rounded-lg shadow-sm space-y-4">', '</div>\n<!-- Hak Tambah Akun -->\n                    <div class="bg-white p-4 border rounded-lg shadow-sm space-y-4">')
...
with open(r"d:\DANIEL\web dev 1\Resource Collection\index.html", "w", encoding="utf-8") as f:
    f.write(html)
